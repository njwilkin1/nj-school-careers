...
export async function generateMetadata({ params }: { params: Promise<{ county: string }> }) {
  ...
  return {
    title: `${countyName} County Teaching Jobs | NJSchoolCareers`,
    description: `Browse school jobs in ${countyName} County, New Jersey. Apply directly with no account required.`,
  };
}
...
export default async function CountyJobsPage({ params }: PageProps) {
  const { county } = await params;
    ...
    <main className="min-h-screen bg-slate-50 px-6 py-12 text-slate-900">
      <div className="mx-auto max-w-5xl">
        <h1 className="text-4xl font-bold tracking-tight">
          {countyName} County Teaching Jobs
        </h1>

        <p className="mt-4 text-slate-600">
          Browse current school job opportunities in {countyName} County, New Jersey.
          ...
          {filteredJobs.length > 0 ? (
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <p className="text-slate-500">
                No {countyName} County jobs available right now.
              </p>
            </div>
          )}
        </div>
